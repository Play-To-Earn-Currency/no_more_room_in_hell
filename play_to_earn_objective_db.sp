...
public void OnObjectiveStart(Event event, const char[] name, bool dontBroadcast)
{
    ...
    for (int i = 0; i < length; i += 1)
    {
        ...
        if (playerObj == null)
        {
            PrintToServer("[PTE] [OnObjectiveStart] ERROR: %d (online index) have any invalid player object: %s", i, onlinePlayers[i]);
            continue;
        }
...
public void OnObjectiveComplete(Event event, const char[] name, bool dontBroadcast)
{
    ...
    {
        PrintToServer("[PTE] WARNING: Objective complete event called, but the 'objectiveInCooldown' is still true...");
        return;
    }
    ...
    {    // Objective calculation
        ...
        for (int i = 0; i < length; i += 1)
        {
            ...
            if (playerObj == null)
            {
                PrintToServer("[PTE] [OnObjectiveComplete] ERROR: %d (online index) have any invalid player object: ", i, onlinePlayers[i]);
                continue;
            }
